...
function changeColor() {
    ...
    for (let i = 0; i < tmpSubjectArrValue.length; i++) {
        p[tmpSubjectArrValue[i].value].style.backgroundColor = colorArr[count % colorArr.length];
        count++;
    }
}
...
function tmpSubject(e) {
    let tmpSubjectArrValue = JSON.parse(localStorage.getItem("tmpSubjectArrValue")) || [];
    let subjectName = e.previousSibling.textContent.trim();
    let value = parseInt(e.value);
    let timeClass = e.className;
    let subject = { name: subjectName, value: value, time: timeClass };
